...
import { clearRedirectPath, setRedirectPath } from '../redux/redirectSlice';
...
function Nav() {
  ...
  const handleMarkAsRead = async (id) => {
    ...
    try {
      await axios.post(
        `${serverUrl}/api/user/notifications/${id}/read`,
        {},
        { withCredentials: true }
      );
  ...
  // ✅ Navbar login should always lead to homepage after login
  const handleNavbarLogin = () => {
    dispatch(setRedirectPath(null)); // means: after login → "/"
    localStorage.removeItem("redirectAfterLogin"); // ⭐ NEW: clear backup
    setShowPro(false);
    setShowHam(false);
    navigate("/login");
  };
  ...
  return (
    <div onClick={closeDropdown}>
      <div className='w-[100%] h-[90px] fixed top-0 px-[20px] py-[10px] flex items-center justify-between bg-[#00000047] backdrop-blur-sm z-[1000]'>
          ...
          <div className='hidden lg:flex items-center gap-4'>
            {userData?.role === 'educator' ? (
              <div className='hidden lg:flex items-center gap-4'>
                ...
                <span
                  className='text-white cursor-pointer hover:text-gray-300 transition-colors'
                  onClick={() => scrollToSection('your-courses')}
                >
                  YourCourses
                </span>
                <span
              ...
              <div className='hidden lg:flex items-center gap-4'>
                <span
                  className='text-white cursor-pointer hover:text-gray-300 transition-colors'
                  onClick={() => scrollToSection('explore-courses')}
                >
                  Home
                </span>
                <span
        ...
        <div className='w-[30%] lg:flex items-center justify-center gap-4 hidden'>
          {!userData ? (
            <>
                ...
                {showPro && (
                  <div className='profile-dropdown absolute top-full right-0 mt-2 w-48 bg-black border-2 border-white rounded-lg shadow-lg z-20'>
                    <div
                      ...
                      onClick={(e) => { e.stopPropagation(); handleNavbarLogin(); }}
                    >
                      Login
                    </div>
                  </div>
              ...
              <div
                className='px-[20px] py-[10px] border-2 lg:border-white border-black lg:text-white bg-black text-white rounded-[10px] text-[18px] font-light cursor-pointer'
            ...
            <>
              ...
              <div className='relative'>
                ...
                {showPro && (
                  <div className='profile-dropdown absolute top-full right-0 mt-2 w-48 bg-black border-2 border-white rounded-lg shadow-lg z-20'>
                    <div
                      className='px-4 py-2 text-white cursor-pointer hover:bg-gray-700 rounded flex items-center gap-2'
                    ...
                    {userData?.role === "student" && (
                      <>
                        ...
                        >
                          <FaCreditCard className='w-[20px] h-[20px]' />{" "}
      ...
      >
        ...
        {!userData ? (
          <IoMdPerson
            className='w-[50px] h-[50px] fill-white cursor-pointer border-[2px] border-[#fdfbfb7a] bg-[#000000d5] rounded-full p-[10px]'
        ...
        {userData?.role === "student" && (
          <span
            className='flex items-center justify-center gap-2 text-[18px] text-white border-[2px] border-[#fdfbfb7a] bg-[#000000d5] rounded-lg px-[65px] py-[20px]'
            onClick={() => { navigate("/wishlist"); setShowHam(false); }}
          >
            <FaHeart className='w-[20px] h-[20px]' /> Wishlist
          </span>
        )}
        ...
        {userData?.role === "admin" ? (
          <div
            className='flex items-center justify-center gap-2 text-[18px] text-white border-[2px] border-[#fdfbfb7a] bg-[#000000d5] rounded-lg px-[60px] py-[20px]'
            onClick={() => { navigate("/admin/dashboard"); setShowHam(false); }}
          >
            Admin Panel
          </div>
        ) : (
